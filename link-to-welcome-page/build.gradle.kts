...
plugins {
  `java-library`
  // Git-derived versioning (YYYYMMDD-<sha7>) and reproducible jars, consistent
  // with the other subprojects.
  id("pdc-versioning")
  // Maven publication to the PDC GitHub Packages registry, consistent with the
  // other subprojects. Publishes org.philanthropydatacommons:link-to-welcome-page
  // from components["java"] (no shadow plugin here, so the unqualified jar is
  // the publishable artifact; there is no -plain classifier to exclude).
  id("pdc-publishing")
  // Test coverage, mirroring twilio-keycloak-provider so `jacocoReport` exists
  // for the CI test step.
  jacoco
}
...
dependencies {
  ...
  // Log via the slf4j API; in Keycloak slf4j is bridged to jboss-logging at
  // runtime. Declared compileOnly so we depend on the API, not the impl.
