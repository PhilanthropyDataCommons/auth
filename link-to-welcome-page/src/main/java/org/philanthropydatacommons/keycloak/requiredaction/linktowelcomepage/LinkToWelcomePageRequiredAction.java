...
package org.philanthropydatacommons.keycloak.requiredaction.linktowelcomepage;
 ...
 * outside the action-token flow (e.g. a normal login), so it is safe to leave enabled realm-wide.
 * See README.md. Stateless; the factory creates a new instance per request.
 */
public final class LinkToWelcomePageRequiredAction implements RequiredActionProvider {
  ...
  public InitiatedActionSupport initiatedActionSupport() {
    // NOT_SUPPORTED: the action runs as a required action added by
    // execute-actions-email. As an AIA it would let a user self-trigger a no-op.
    return InitiatedActionSupport.NOT_SUPPORTED;
  }

  @Override
  public void evaluateTriggers(RequiredActionContext context) {
    // Added to a user explicitly; never auto-triggers.
  }
  ...
  private void applyLink(RequiredActionContext context) {
    ...
    String welcomePageUri =
        ResolveRelative.resolveRelativeUri(context.getSession(), rootUrl, baseUrl);
    // resolveRelativeUri returns baseUrl unchanged (null when blank), so a
    // Root-URL-only client resolves to nothing: fall back to the root URL.
    if ((welcomePageUri == null || welcomePageUri.isBlank())
        && rootUrl != null && !rootUrl.isBlank()) {
      welcomePageUri = ResolveRelative.resolveRootUrl(context.getSession(), rootUrl);
    }
    if (welcomePageUri == null || welcomePageUri.isBlank()) {
  ...
  /**
   * Why the action did not present a link, with the log level for its message: the expected no-op
   * (outside the action-token flow) is DEBUG; genuine misconfigurations are WARN, so an admin who
   * accidentally enables this as a realm default does not flood the logs.
   *
   * <p>Each template is a single physical line ({@code +}-concatenated, not a text block) so each
   * {@code LOGGER} call emits one line for line-oriented ingestion. Package-private so the test in
   * this package can exercise the production {@code render} rather than a copy of the template.
   */
  enum Outcome {
    NO_TARGET_CLIENT(
