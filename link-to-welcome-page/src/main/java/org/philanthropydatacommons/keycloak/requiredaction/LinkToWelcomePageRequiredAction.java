...
public final class LinkToWelcomePageRequiredAction implements RequiredActionProvider {
  ...
  private void applyLink(RequiredActionContext context) {
    ...
    if (welcomePageUri != null && !welcomePageUri.isBlank()) {
      ...
      String resolved =
          RedirectUtils.verifyRedirectUri(context.getSession(), welcomePageUri, client);
      if (resolved == null) {
        logNoop(context, Outcome.INVALID_REDIRECT_URI, welcomePageUri);
        return;
      }
      // Defense in depth: at 26.7.2 verifyRedirectUri accepts a non-http(s)
      // scheme when the client has registered one (e.g. a mobile-app scheme),
      // so re-enforce http(s) before the URI reaches info.ftl's href.
      if (!isAllowedRedirectScheme(resolved)) {
        logNoop(context, Outcome.DISALLOWED_SCHEME, resolved);
        return;
      }
  ...
  static boolean isAllowedRedirectScheme(String uri) {
