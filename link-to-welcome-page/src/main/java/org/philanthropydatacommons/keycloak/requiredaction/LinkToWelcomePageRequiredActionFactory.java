...
/**
 * Factory for {@link LinkToWelcomePageRequiredAction}. Declares the two config properties shown in
 * the Admin Console's Required Action configuration dialog, and validates the welcome-page URI
 * against the target client's registered valid redirect URIs at save time.
 */
public final class LinkToWelcomePageRequiredActionFactory implements RequiredActionFactory {
  ...
  // Keycloak's own keys.
  private static final String MSG_TARGET_CLIENT_REQUIRED =
  ...
  private static final String MSG_INVALID_REDIRECT_URI =
      "link-to-welcome-page.error-invalid-redirect-uri";
  ...
  private static final ProviderConfigProperty TARGET_WELCOME_PAGE_URI_PROP =
      new ProviderConfigProperty(
  ...
  public void validateConfig(
    ...
    String targetWelcomePageUri =
        model.getConfigValue(LinkToWelcomePageRequiredAction.CONFIG_TARGET_WELCOME_PAGE_URI);
    ...
    // saved. When blank, the client's home URI is used at run time. At 26.7.2
    // verifyRedirectUri accepts a non-http(s) scheme when the client has
    // registered one, so re-enforce http(s) at save time too.
    if (!welcomePageBlank) {
      String verified = RedirectUtils.verifyRedirectUri(session, targetWelcomePageUri, client);
      if (verified == null
          || !LinkToWelcomePageRequiredAction.isAllowedRedirectScheme(verified)) {
        throw new ValidationException(
            new ValidationError(
                getId(),
                LinkToWelcomePageRequiredAction.CONFIG_TARGET_WELCOME_PAGE_URI,
                MSG_INVALID_REDIRECT_URI,
                targetWelcomePageUri,
                targetClient));
      }
    }
  }
