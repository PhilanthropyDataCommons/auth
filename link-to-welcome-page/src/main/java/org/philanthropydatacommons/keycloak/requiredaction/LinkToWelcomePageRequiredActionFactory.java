 ...
 * config properties (target client + target welcome-page URI) shown in the
 * Admin Console's Required Action configuration dialog, and validates the
 * welcome-page URI against the target client's registered valid redirect URIs
 * at save time.
 *
 * @author GLM-5.2
 */
public final class LinkToWelcomePageRequiredActionFactory implements RequiredActionFactory {
    ...
    /**
     * Message keys for the config-validation errors. These are translated by
     * the admin theme's message bundle (see the companion pdc-keycloak-theme
     * admin messages); ValidationError.message is a message key, not a literal
     * (see Keycloak's ValidationError: "Holds the message key for
     * translation"). Use kebab-case to match Keycloak's own keys
     * (error-invalid-value, error-invalid-uri, ...).
     */
    ...
    public RequiredActionProvider create(KeycloakSession session) {
        // Keycloak's own required-action factories return a fresh provider per
        // call (e.g. WebAuthnRegisterFactory.create returns
        // new WebAuthnRegister(session, ...)); AuthenticationManager calls
        // factory.create(session) per request (AuthenticationManager ~line
        // 1315). This provider is stateless, so a new instance per call matches
        // Keycloak's convention without cost.
    ...
    public List<ProviderConfigProperty> getConfigMetadata() {
        // Keep the default max_auth_age property inherited from RequiredActionFactory
        // and add our two properties, so the config button exposes all three.
    ...
    public void validateConfig(KeycloakSession session, RealmModel realm, RequiredActionConfigModel model) {
        // Validate the inherited max_auth_age first.
        ...
        // Both blank is allowed: the action is a no-op until configured.
        if (clientBlank && welcomePageBlank) {
            return;
        }
        // The target client is required whenever any configuration is present.
        if (clientBlank) {
        ...
        // The welcome-page URI is optional. When set, it must be registered as
        // a valid redirect URI for the target client (the same check Keycloak
        // applies to execute-actions-email), so an open/misconfigured target
        // cannot be saved. When blank, the client's home URI is used at run
        // time (no validation needed; it is the client's own URL).
