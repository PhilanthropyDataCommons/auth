...
public class LinkToWelcomePageRequiredActionTest {
  ...
  void setsConfiguredWelcomePageUriAndAuthNotes() {
    ...
    when(config.getConfigValue(LinkToWelcomePageRequiredAction.CONFIG_TARGET_WELCOME_PAGE_URI))
          ...
          .when(
              () ->
                  RedirectUtils.verifyRedirectUri(
                      eq(session), eq("https://example.com/"), eq(client)))
          .thenReturn("https://example.com/");
  ...
  void noOpWhenClientHomeUriResolvesToNull() {
    ...
    try (MockedStatic<ResolveRelative> resolveRelative =
      ...
      resolveRelative.verify(
          () -> ResolveRelative.resolveRelativeUri(eq(session), eq(null), eq(null)));
    }
  ...
  @Test
  void noOpWhenConfiguredWelcomePageUriIsAnInvalidRedirectUri() {
    when(context.getConfig()).thenReturn(config);
    when(config.getConfigValue(LinkToWelcomePageRequiredAction.CONFIG_TARGET_CLIENT))
        .thenReturn("my-client");
    when(context.getRealm()).thenReturn(realm);
    ...
    try (MockedStatic<RedirectUtils> redirectUtils = Mockito.mockStatic(RedirectUtils.class)) {
      redirectUtils
          .when(
              () ->
                  RedirectUtils.verifyRedirectUri(
                      eq(session), eq("javascript:alert(document.cookie)//"), eq(client)))
          .thenReturn(null);
  ...
  void resolvesRelativeConfiguredUriAgainstClientAtRuntime() {
    ...
    try (MockedStatic<RedirectUtils> redirectUtils = Mockito.mockStatic(RedirectUtils.class)) {
      redirectUtils
          .when(() -> RedirectUtils.verifyRedirectUri(eq(session), eq("/welcome"), eq(client)))
          .thenReturn("https://example.com/welcome");

      action.requiredActionChallenge(context);

      redirectUtils.verify(
          () -> RedirectUtils.verifyRedirectUri(eq(session), eq("/welcome"), eq(client)));
    }
  ...
  void noOpWhenDefaultHomeUriHasDisallowedScheme() {
    // The default home URI is not redirect-URI-validated, so a non-http(s)
    // scheme resolved from rootUrl/baseUrl must be rejected before info.ftl.
    when(context.getConfig()).thenReturn(config);
    when(config.getConfigValue(LinkToWelcomePageRequiredAction.CONFIG_TARGET_CLIENT))
        .thenReturn("my-client");
    when(context.getRealm()).thenReturn(realm);
  ...
  @Test
  void noOpWhenConfiguredWelcomePageUriResolvesToDisallowedScheme() {
    // At 26.7.2 verifyRedirectUri accepts a non-http(s) scheme when the client
    // has registered one, so the run-time path must re-enforce http(s) before
    // the URI reaches info.ftl's href.
    when(context.getConfig()).thenReturn(config);
    when(config.getConfigValue(LinkToWelcomePageRequiredAction.CONFIG_TARGET_CLIENT))
        .thenReturn("my-client");
    when(context.getRealm()).thenReturn(realm);
  ...
  void outcomeTextBlocksRenderAsSingleLineWithSingleSpaces() {
    // Pins the rendered form so a whitespace edit cannot silently regress
    // the slf4j pattern: the trailing-space-before-backslash continuations
    // produce a single line with single spaces.
    assertEquals(
