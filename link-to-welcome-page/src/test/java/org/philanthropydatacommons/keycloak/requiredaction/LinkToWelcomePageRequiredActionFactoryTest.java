 ...
 * Author: GLM-5.2
 *
 * Tests for LinkToWelcomePageRequiredActionFactory.validateConfig. The
 * RedirectUtils.verifyRedirectUri static check is stubbed with
 * Mockito.mockStatic (Mockito 5.x default inline mock maker).
 */
...
public class LinkToWelcomePageRequiredActionFactoryTest {
    ...
    /** Stubs the config values the validator reads (including the inherited max_auth_age). */
    private void stubConfig(String targetClient, String targetWelcomePageUri) {
        // super.validateConfig reads max_auth_age; stub it null so the inherited
        // check is a no-op and strict stubbing sees a matching stub for that call.
