...
5. **Create a dedicated client for this link** (if you do not already have one).
   ...
   - **Client authentication**: **Off** (public). This client never receives
     an authorization code; it exists only to own the welcome-page URL.
...
(`org.philanthropydatacommons.keycloak.requiredaction.linktowelcomepage.LinkToWelcomePageRequiredAction`)
...
To run the local verification CI enforces — Checkstyle, Error Prone with
NullAway, and the JUnit test suite — use:

```sh
./gradlew :link-to-welcome-page:check
```

`check` does not run JaCoCo coverage (the `jacoco` plugin is applied but
`jacocoTestReport` is not wired into `check`, and the report task is skipped
when there is no execution data). To produce the HTML coverage report CI
produces, run the tests and the report task in one invocation:

```sh
./gradlew :link-to-welcome-page:test :link-to-welcome-page:jacocoTestReport
```

The report is written to `link-to-welcome-page/build/reports/jacoco/test/html/`.
...
`compileOnly`/`testImplementation` Keycloak jars at 26.7.3 (matches `main`).
