...
/**
 * Requires a mobile number from the user during authentication.
 *
...
public class MobileNumberRequiredAction implements RequiredActionProvider {
  ...
  public void processAction(RequiredActionContext context) {
    ...
    if (Validation.isBlank(mobileNumber) || mobileNumber.length() < 5) {
      context.challenge(
          createForm(
              context,
              form -> form.addError(new FormMessage(MOBILE_NUMBER_FIELD, "Invalid input"))));
      return;
    }
  ...
  private Response createForm(
      RequiredActionContext context, @Nullable Consumer<LoginFormsProvider> formConsumer) {
